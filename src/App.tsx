...
import { herald } from './agents/herald';
...
const IONIAN_FREQS = [65.41, 98.00, 130.81, 164.81, 196.00];
void IONIAN_FREQS; // retained for audio parity reference
...
export default function App() {
  ...
  const handleHeraldChat = async () => {
    ...
    trace.lifecycle('start', { trigger: 'user_message', agent: 'herald' });

    const text = await herald.sendMessage(userMsg, {
      consensus: stateRef.current.consensus,
      spread: stateRef.current.spread,
      truth: stateRef.current.truth,
      info: stateRef.current.info,
    });
  ...
  useEffect(() => {
    ...
    const starsMat = new THREE.PointsMaterial({
      color: THEME_BLUE, size: 0.12, transparent: true, opacity: 0.5, depthWrite: false, depthTest: true
    });
    ...
    for (let i = 0; i < WORKER_COUNT; i++) {
      const workerMat = new THREE.MeshStandardMaterial({
        color: THEME_BLUE, emissive: THEME_BLUE, emissiveIntensity: 2,
        transparent: true, opacity: 0.9, depthWrite: false
      });
      const worker = new THREE.Mesh(workerGeom, workerMat);

      const wireMat = new THREE.MeshBasicMaterial({
        color: THEME_BLUE, wireframe: true, transparent: true, opacity: 0.4,
    ...
    const filamentMat = new THREE.MeshBasicMaterial({
      color: THEME_BLUE, transparent: true, opacity: 0.1,
      blending: THREE.AdditiveBlending, depthWrite: false, depthTest: true
    });
    ...
    const shieldMat = new THREE.MeshBasicMaterial({
      color: DISSONANCE_RED, wireframe: true, transparent: true, opacity: 0, depthWrite: false, depthTest: true
    });
    ...
    const torusMat = new THREE.MeshBasicMaterial({ color: THEME_BLUE, transparent: true, opacity: 0, depthWrite: false, depthTest: true });
    ...
    for (let i = 0; i < SPIRAL_NODES; i++) {
      ...
      spiralPoints.push(new THREE.Vector3(Math.cos(t) * r, Math.sin(t) * r, Math.sin(t / PHI) * 0.5));
      spiralPoints2.push(new THREE.Vector3(Math.cos(t) * r, Math.sin(t) * r, -Math.sin(t / PHI) * 0.5));
    }
    ...
    const spiralMat = new THREE.LineBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0, depthWrite: false, depthTest: true });
    ...
    const sparkMat = new THREE.PointsMaterial({
      color: DISSONANCE_RED, size: 0.08, transparent: true,
      blending: THREE.AdditiveBlending, opacity: 0, depthWrite: false, depthTest: true
    });
    ...
    const bokehPass = new BokehPass(scene, camera, { focus: 10.0, aperture: 0.001, maxblur: 0.01 });
    composer.addPass(bokehPass);
    const bloomPass = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 0.05, 0.4, 0.2);
    ...
    const animate = () => {
      ...
      tracer.position.set(Math.sin(tt * 0.7) * 5, Math.cos(tt * 0.5) * 3, Math.sin(tt * 0.3) * 5);

      const tracerDist = camera.position.distanceTo(tracer.position);
      bokehPass.uniforms['focus'].value = THREE.MathUtils.lerp(bokehPass.uniforms['focus'].value, tracerDist, 0.05);
      ...
      workers.forEach((w, i) => {
        ...
        const targetY = Math.sin(t * 0.5 * config.speed + config.floatOffset) * (1.0 - consensus * 0.7) * 2;

        w.position.lerp(_scratchVec.set(targetX, targetY, targetZ), 0.1 * Math.max(0.2, config.speed));
        ...
        if (isDissonant) {
          stateColor = 0xff1100; stateIntensity = 80 + Math.sin(t * 120) * 40; jitter = 0.25; stateScale = 1.3 + Math.sin(t * 40) * 0.1;
        } else if (isConverging) {
          stateColor = 0x00ffff; stateIntensity = 30 + Math.sin(t * 20) * 15; stateScale = 1.15 + Math.sin(t * 10) * 0.08;
        } else if (isIdle) {
          stateColor = 0x243447; stateIntensity = 0.5 + truth * 1.0; stateScale = 0.75 + Math.sin(t * 1.5) * 0.03;
        } else if (isActive) {
          stateColor = THEME_BLUE; stateIntensity = 12 + truth * 15 + Math.sin(t * 8) * 5; jitter = 0.02 * config.speed; stateScale = 1.0 + Math.sin(t * 4) * 0.02;
        }
        ...
        if (agentTruth > 0.8 || isConverging) {
          halo.position.copy(w.position);
          const haloPulse = Math.sin(t * 4 + i) * 0.5 + 0.5;
          halo.scale.setScalar(w.scale.x * (1.3 + haloPulse * 0.6));
        ...
        (data.wire as THREE.Mesh && (data.wire as THREE.Mesh).material as THREE.MeshBasicMaterial);
        ((data.wire as THREE.Mesh).material as THREE.MeshBasicMaterial).color.setHex(stateColor);

        const activationWave = Math.sin(t * 2 - i * 0.5) * 0.5 + 0.5;
        const mat = w.material as THREE.MeshStandardMaterial;
        mat.emissive.setHex(selectedAgentIndex === i ? 0xffffff : stateColor);
        mat.color.setHex(stateColor);
        mat.emissiveIntensity = stateIntensity * (0.5 + activationWave * 0.5) * (0.2 + config.truthScore * 1.5) * (selectedAgentIndex === i ? 2 : 1);
        ...
        if (agentTruth > TRUTH_THRESHOLD && Math.random() > 0.99) {
          ...
          gsap.to(mat.emissive, { r: selectedAgentIndex === i ? 1.0 : 0.3, g: selectedAgentIndex === i ? 1.0 : 0.72, b: 1.0, duration: 1.0 });
        }

        const nextW = workers[(i + 1) % WORKER_COUNT];
        const direction = new THREE.Vector3().subVectors(nextW.position, w.position);
        dummy.position.copy(w.position);
        dummy.lookAt(nextW.position);
        dummy.scale.set(1, 1, direction.length());
        dummy.updateMatrix();
      ...
      for (let i = 0; i < workers.length; i++) {
        for (let j = i + 1; j < workers.length; j++) {
          const w1 = workers[i], w2 = workers[j];
          const dist = w1.position.distanceTo(w2.position);
          if (dist < 1.2 && linkIdx < MAX_LINKS) {
            const c1 = workerConfigs[i] || { truthScore: 0.5 };
            const c2 = workerConfigs[j] || { truthScore: 0.5 };
            const truthDiff = Math.abs(c1.truthScore - c2.truthScore);
            const p1 = w1.position, p2 = w2.position;
            linkPositions[linkIdx * 6]     = p1.x; linkPositions[linkIdx * 6 + 1] = p1.y; linkPositions[linkIdx * 6 + 2] = p1.z;
            linkPositions[linkIdx * 6 + 3] = p2.x; linkPositions[linkIdx * 6 + 4] = p2.y; linkPositions[linkIdx * 6 + 5] = p2.z;
            ...
            linkColors[linkIdx * 6]     = r * alpha; linkColors[linkIdx * 6 + 1] = g * alpha; linkColors[linkIdx * 6 + 2] = b * alpha;
            linkColors[linkIdx * 6 + 3] = r * alpha; linkColors[linkIdx * 6 + 4] = g * alpha; linkColors[linkIdx * 6 + 5] = b * alpha;
            linkIdx++;
            if (dist < 0.6) {
              const cooldown = truthDiff > 0.7 ? 0.2 : 0.8;
              if (truthDiff > 0.4 && now - w1.userData.lastDissonance > cooldown && now - w2.userData.lastDissonance > cooldown) {
                ...
                triggerSpark(midPoint, Math.floor(20 + (truthDiff - 0.4) * 40));
                ...
                m1.emissive.setHex(0xff3300); m2.emissive.setHex(0xff3300);
      ...
      for (let i = 0; i < sparkCount; i++) {
        if (sparkLifetimes[i] > 0) {
          ...
          activeSparks = true;
        } else { positions[i * 3] = 1000; }
      }
      if (activeSparks) { sparkGeom.attributes.position.needsUpdate = true; sparkMat.opacity = 1.0; }
      else { sparkMat.opacity = 0; }

      starfield.rotation.y += 0.001 + consensus * 0.05;
      starfield.scale.setScalar(1 + consensus * 0.5);
      starsMat.opacity = 0.3 + Math.sin(t * 4) * 0.2;

      audioEngine.update(consensus, (1.0 - consensus) * 12.0);
      ...
      if (analyzerCanvasRef.current) {
        ...
        if (ctx) {
          ...
          for (let i = 0; i < bufferLength; i++) {
            ...
            ctx.fillStyle = `hsla(${200 + modalFreq * 10}, 100%, 60%, ${0.3 + barHeight / canvas.height})`;
      ...
      sentinelMat.emissiveIntensity = 0.5 + consensus * 5 * truth;
      const hue = THREE.MathUtils.lerp(0.5, 0.7, truth);
      sentinelMat.color.setHSL(hue, 0.8, 0.5);
      sentinelMat.emissive.setHSL(hue, 0.8, 0.5);

      shield.visible = dissonanceCount > 0 && t % 2 < 0.1;
      if (shield.visible) shieldMat.opacity = Math.sin(t * 10) * 0.2;

      bloomPass.strength = 0.05 + consensus * 0.1 * truth;
      controls.update();
      composer.render();
    };
    ...
    return () => {
      ...
      controls.dispose();
      renderer.dispose();
      sentinelGeom.dispose(); sentinelMat.dispose();
      starsGeom.dispose(); starsMat.dispose();
      workerGeom.dispose(); haloGeom.dispose();
      filamentGeom.dispose(); filamentMat.dispose();
      instancedFilaments.dispose();
      torusGeom.dispose(); torusMat.dispose();
      spiralGeom.dispose(); spiralGeom2.dispose(); spiralMat.dispose();
      linkGeom.dispose(); linkMat.dispose();
      sparkGeom.dispose(); sparkMat.dispose();
      tracerGeom.dispose(); tracerMat.dispose();
      shieldGeom.dispose(); shieldMat.dispose();
      if (containerRef.current) containerRef.current.removeChild(renderer.domElement);
      if (audioEngineRef.current) { audioEngineRef.current.dispose(); audioEngineRef.current = null; }
    };
  }, []);

  const triggerBreakthrough = () => {
    setIsBreakthrough(true);
    gsap.to({ val: consensus }, {
      val: 1.0, duration: 2,
      onUpdate: function () { setConsensus(this.targets()[0].val); },
  ...
  return (
    <div className="relative w-full h-screen bg-[#00050a] text-[#4db8ff] font-sans overflow-hidden">
        ...
        <div className="flex flex-col gap-4 overflow-y-auto custom-scrollbar pointer-events-auto pr-2 pb-8">
          ...
          <div className="p-6 hud-glass flex flex-col h-[400px] shrink-0">
            <div className="flex items-center gap-3 mb-6 border-b border-[#4db8ff]/10 pb-4">
              <div className="p-2 bg-[#4db8ff]/10 rounded-lg"><Bot className="w-5 h-5 text-[#4db8ff]" /></div>
              <div>
            ...
            <div className="mt-4 flex gap-2">
              ...
              />
              <button onClick={handleHeraldChat} disabled={isHeraldThinking}
          ...
          <div className="p-6 hud-glass shrink-0">
            <div className="flex items-center gap-3 mb-6">
              <div className="p-2 bg-[#4db8ff]/10 rounded-lg"><Activity className="w-5 h-5" /></div>
            ...
            <div className="space-y-6">
              ...
              <button onClick={() => { triggerBreakthrough(); audioEngineRef.current?.resume(); }}
      ...
      <div className="absolute bottom-8 right-8 z-20 w-80 p-6 hud-glass pointer-events-auto">
        <div className="flex items-center gap-3 mb-6">
          <div className="p-2 bg-[#4db8ff]/10 rounded-lg"><Shield className="w-5 h-5" /></div>
        ...
        <div className="space-y-4">
          ...
          {selectedAgentIndex !== null && (
            <div className="pt-4 space-y-6 border-t border-[#4db8ff]/10">
              {[
                { label: `Agent ${selectedAgentIndex + 1} Truth`, val: (workerConfigs[selectedAgentIndex].truthScore * 100).toFixed(0) + '%', min: 0, max: 1, step: 0.01, value: workerConfigs[selectedAgentIndex].truthScore, onChange: (v: number) => { setWorkerConfigs(prev => prev.map((c, idx) => idx === selectedAgentIndex ? { ...c, truthScore: v } : c)); audioEngineRef.current?.resume(); } },
                { label: `Agent ${selectedAgentIndex + 1} Speed`, val: workerConfigs[selectedAgentIndex].speed.toFixed(2), min: 0, max: 2, step: 0.01, value: workerConfigs[selectedAgentIndex].speed, onChange: (v: number) => { setWorkerConfigs(prev => prev.map((c, idx) => idx === selectedAgentIndex ? { ...c, speed: v } : c)); audioEngineRef.current?.resume(); } },
                { label: `Agent ${selectedAgentIndex + 1} Offset`, val: workerConfigs[selectedAgentIndex].floatOffset.toFixed(1), min: 0, max: 20, step: 0.1, value: workerConfigs[selectedAgentIndex].floatOffset, onChange: (v: number) => { setWorkerConfigs(prev => prev.map((c, idx) => idx === selectedAgentIndex ? { ...c, floatOffset: v } : c)); audioEngineRef.current?.resume(); } }
              ].map(({ label, val, min, max, step, value, onChange }) => (
                <div key={label} className="space-y-3">
                  <div className="flex justify-between text-[10px] font-bold tracking-tighter uppercase opacity-60">
                    <span>{label}</span><span>{val}</span>
          ...
          {selectedAgentIndex === null && (
            <div className="py-8 text-center opacity-30 italic text-[10px]">Select an agent to tune parameters</div>
          )}
        </div>
      </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 w-full max-w-md p-4 hud-glass pointer-events-auto">
        ...
        <canvas ref={analyzerCanvasRef} width={300} height={60} className="w-full h-12 mb-4 opacity-80" />
...
function StatCard({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center gap-4 px-4 py-3 hud-glass min-w-[140px] pointer-events-auto">
      <div className="p-1.5 bg-[#4db8ff]/10 rounded-md text-[#4db8ff]">{icon}</div>
      <div>
